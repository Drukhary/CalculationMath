...
import numpy
import numpy as np
from util.RedConsts import condition, x
...
def fault(derivative_power, power, denominator):
    ...
    def fault(r, red_function, a, b):
        ...
        try:
            n = (max([abs(the_derivative.subs({x: i}).n()) for i in np.arange(a, b + EPSILON / 2, EPSILON)])
...
class Method(enum.Enum):
    ...
    RECTANGLE = (
        ...
        2, 'center modify')
    RECTANGLE_LEFT = (
        rectangle_method_factory(Mode.LEFT), fault(derivative_power=1, power=2, denominator=2), "Rectangle method",
        'left modify')
    RECTANGLE_RIGHT = (
    ...
    def approximate_calculation(self, red_function, a, b, eps=0.01, n=4):
        tables = []
        I_0, table = self.num_method(red_function, a, b, eps, n)
        table = '\nfor n = {}\nI = {}\n'.format(n, I_0) + table
        tables.append(table)
        n *= 2
        I_1, table = self.num_method(red_function, a, b, eps, n)
        table = 'for n = {}\n' \
                'I = {} ({})\n' \
                '|I_1 - I_0| = {} ({})\n' \
                    .format(n, RedRound(I_1, eps), I_1, RedRound(abs(I_1 - I_0), eps/100), abs(I_1 - I_0)) + table
        tables.append(table)
        while abs(I_1 - I_0) > eps:
            n *= 2
            I_0, (I_1, table) = I_1, self.num_method(red_function, a, b, eps, n)
            table = 'for n = {}\n' \
                    'I = {} ({})\n' \
                    '|I_1 - I_0| = {} ({})\n' \
                        .format(n,RedRound(I_1, eps), I_1, RedRound(abs(I_1 - I_0), eps/100), abs(I_1 - I_0)) + table
    ...
    def n_num_method(self, red_function, a, b, epsilon=0.01):
        ...
        for i in x:
            temp = int(math.ceil(self.fault(epsilon / len(x), red_function, i[0], i[-1])))
            if temp < 2: temp = 2
            n.append(temp)
        for i in range(len(n)):
            if n[i] is not None:
                _x_ = np.arange(x[i][0], x[i][-1] + (x[i][-1] - x[i][0]) / n[i] / 2, (x[i][-1] - x[i][0]) / n[i])
    ...
    def find_intervals(self, the_function, a, b, step):
        args = numpy.arange(a, b + step / 2, step)
